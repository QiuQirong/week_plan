...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/week") {
      try {
        const payload = await buildWeekPayload(env);
        return json(payload);
      } catch (error) {
        console.error(error);
        return json(
...
async function buildWeekPayload(env) {
...
async function queryDataSource(notion, dataSourceId) {
  const results = [];
  let cursor;
...
function getWeekWindow(now = new Date(), timeZone = "Asia/Shanghai") {
...
function dateKeyInTimeZone(date, timeZone = "Asia/Shanghai") {
  return zonedDateParts(date, timeZone).dateKey;
}

function formatWeekLabel(start, end, timeZone = "Asia/Shanghai") {
  return `${zonedDateParts(start, timeZone).dateLabel} - ${zonedDateParts(end, timeZone).dateLabel}`;
}

function formatDateTimeLabel(date, timeZone = "Asia/Shanghai") {
  const parts = zonedDateParts(date, timeZone);
  return `${parts.dateLabel} ${String(parts.hour).padStart(2, "0")}:${String(parts.minute).padStart(2, "0")}`;
}

function formatClockLabel(date, timeZone = "Asia/Shanghai") {
  const parts = zonedDateParts(date, timeZone);
  return `${String(parts.hour).padStart(2, "0")}:${String(parts.minute).padStart(2, "0")}`;
}

function minutesIntoDay(date, timeZone = "Asia/Shanghai") {
  const parts = zonedDateParts(date, timeZone);
  return parts.hour * 60 + parts.minute;
}

function combineDateAndClock(dateKey, clock, timeZone = "Asia/Shanghai") {
